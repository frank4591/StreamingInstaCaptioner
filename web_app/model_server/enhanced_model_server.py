#!/usr/bin/env python3
"""
Enhanced Model Server with Video Context Integration

This server integrates the video context extraction functionality from
VideoContextImageCaptioning project into the streaming captioner.
"""
...
def load_enhanced_model():
    """Load the LFM2-VL model with video context capabilities."""
    global model, processor, video_context_pipeline, device
    ...
    try:
        ...
        # Initialize video context pipeline if available
        if VIDEO_CONTEXT_AVAILABLE:
            logger.info("Initializing video context pipeline...")
            video_context_pipeline = VideoContextCaptionPipeline(
            ...
            logger.info("✅ Video context pipeline initialized")
        else:
            logger.warning("Video context pipeline not available")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("✅ Enhanced Model Server ready!")
    
    yield
    
    logger.info("🛑 Shutting down Enhanced Model Server...")

# Create FastAPI app
app = FastAPI(
    title="Enhanced Streaming Instagram Captioner API",
    description="API for real-time Instagram caption generation with video context",
    version="4.0.0",
    lifespan=lifespan
)
...
async def health_check():
    ...
    return {
        ...
        "device": device,
        "video_context_available": VIDEO_CONTEXT_AVAILABLE and video_context_pipeline is not None,
        "timestamp": time.time()
    }
...
async def generate_video_context_caption(request: VideoContextRequest):
    ...
    try:
        if not VIDEO_CONTEXT_AVAILABLE or video_context_pipeline is None:
            raise HTTPException(
                status_code=503, 
                detail="Video context functionality not available"
            )
        ...
        if frame_descriptions:
            # Simple aggregation - combine unique descriptions
